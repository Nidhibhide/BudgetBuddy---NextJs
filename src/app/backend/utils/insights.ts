...
export async function getDailyAverageInsight(
  ...
  const converted = await convertFromINR(average, user.currency); // Convert currency

  // Build insight message
  return `Your daily average expense is ${user.currency} ${converted.toFixed(2)}.`;
}
...
export async function getTopCategoryInsight(
  ...
  if (result.length > 0) {
    ...
    return `Top category: ${data._id} with ${user.currency} ${convertedTotal.toFixed(2)}.`;
  } else {
...
export async function getTotalExpenseInsight(
  ...
  const sum = result[0]?.total || 0; // Total expenses
  const converted = await convertFromINR(sum, user.currency); // Convert currency

  // Build insight message
  return `Your total expenses this month are ${user.currency} ${converted.toFixed(2)}.`;
}
...
export async function getTotalIncomeInsight(
  ...
  // Build insight message
  return `Your total income this month is ${user.currency} ${converted.toFixed(2)}.`;
}
...
export async function getSavingsInsight(
  ...
  const converted = await convertFromINR(Math.abs(savings), user.currency); // Convert absolute value

  // Build insight message
  if (savings > 0) {
    return `You saved ${user.currency} ${converted.toFixed(2)} this month.`;
  } else if (savings < 0) {
    return `You overspent by ${user.currency} ${converted.toFixed(2)} this month.`;
  } else {
