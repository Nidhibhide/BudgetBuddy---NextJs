  ...
  return (
    <div className="w-full flex flex-col gap-1 relative">
      ...
      <Textarea
        ...
        id={name}
        placeholder={placeholder || label}
        className="placeholder:text-base text-lg text-foreground resize-none min-h-20"
      />
      {meta.touched && meta.error && (
        <p className="text-red-500 text-sm">{meta.error}</p>
      )}
...
export const Button: React.FC<ButtonProps> = ({
  ...
  return (
    <button
      ...
      className={`h-11 flex items-center justify-center ${bgColor} text-white py-2 text-base cursor-pointer font-medium rounded-xl ${hoverColor} hover:shadow-md transition duration-500 ${width} ${className} ${
...
export const GetStartedLink: React.FC<GetStartedLinkProps> = ({
  ...
  return (
    <Link
      href={href}
      onClick={onClick}
      className={`h-11 flex items-center justify-center bg-btn-background py-2.5 px-4 text-center text-base font-medium text-white rounded-xl hover:bg-btn-hover hover:shadow-md transition duration-500 ${width} ${
        loading ? "cursor-not-allowed opacity-50" : ""
      }`}
    >
      {loading ? (
